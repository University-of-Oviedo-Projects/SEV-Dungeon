...
void GameLayer::init() {
	...
	ataquesHielo = 0; 
	vidasJugador = 3;
	municionCogida = 10; // Son los disparos del jugador
...
bool GameLayer::checkLevelCompletion() {
	if (door->isPuzzleCompleted(boxes) && door->isOverlap(player)) {
		...
		pause = true; game->layer = game->gameLayer2; game->gameLayer2->init(); return true;
	}
	return false;
}

void GameLayer::updateGameObjects() {
	for (auto const& enemy : enemies) {
		enemy->update();
...
void GameLayer::handleEnemyProjectiles() {
...
void GameLayer::handleEnemySeeingPlayer() {
	for (auto const& enemy : enemies) {
		if (auto enemy2 = dynamic_cast<Enemy2*>(enemy)) {
...
void GameLayer::handleBoxCollisions() {
	for (auto& box : boxes) {
...
void GameLayer::handlePlayerTileCollisions() {
...
void GameLayer::resolvePlayerTileCollision(Tile* tile) {
...
void GameLayer::handleEnemyTileCollisions() {
...
void GameLayer::resolveEnemy1TileCollision(Enemy1* enemy, Tile* tile) {
...
void GameLayer::resolveEnemy2TileCollision(Enemy2* enemy, Tile* tile) {
	float collisionScale = 0.20f;
	float adjustedWidth = enemy->width * collisionScale;
	float adjustedHeight = enemy->height * collisionScale;
...
void GameLayer::handlePlayerCollisions() {
...
void GameLayer::handleCollectables() {
	...
	for (auto const& municion : municiones) {
		if (player->isOverlap(municion)) {
			if (std::find(deleteMuniciones.begin(), deleteMuniciones.end(), municion)
				== deleteMuniciones.end()) {deleteMuniciones.push_back(municion);}
			municionCogida++;
	...
	for (auto const& curacion : curaciones) {
		if (player->isOverlap(curacion)) {
			if (std::find(deleteCuraciones.begin(), deleteCuraciones.end(), curacion)
				== deleteCuraciones.end()) {deleteCuraciones.push_back(curacion);}
...
void GameLayer::handleProjectileCollisions() {
