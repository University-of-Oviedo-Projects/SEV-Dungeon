...
GameLayer3::GameLayer3(Game* game)
	: Layer(game) {
	pause = true;
...
void GameLayer3::init() {
	...
	ataquesHielo = 0;
	vidasJugador = 1;
	municionCogida = 2; // Son los disparos del jugador
...
void GameLayer3::loadMap(string name) {
	cout << name << endl;
...
void GameLayer3::loadMapObject(char character, float x, float y)
{
	switch (character) {
	case 'P': {  // Puerta
...
void GameLayer3::processControls() {
	// Obtener controles
	SDL_Event event;
...
void GameLayer3::mouseToControls(SDL_Event event) {
...
void GameLayer3::gamePadToControls(SDL_Event event) {
...
bool GameLayer3::checkLevelCompletion() {
	if (door->isPuzzleCompleted(boxes) && door->isOverlap(player)) {
		...
		pause = true; game->layer = game->winLayer; game->layer->init();  return true;
	}
	return false;
}

void GameLayer3::updateGameObjects() {
	for (auto const& enemy : enemies) {
		enemy->update();
...
void GameLayer3::handleEnemyProjectiles() {
...
void GameLayer3::handleEnemySeeingPlayer() {
	for (auto const& enemy : enemies) {
		if (auto enemy2 = dynamic_cast<Enemy2*>(enemy)) {
...
void GameLayer3::handleBoxCollisions() {
	for (auto& box : boxes) {
...
void GameLayer3::handlePlayerTileCollisions() {
...
void GameLayer3::resolvePlayerTileCollision(Tile* tile) {
...
void GameLayer3::handleEnemyTileCollisions() {
...
void GameLayer3::resolveEnemy1TileCollision(Enemy1* enemy, Tile* tile) {
...
void GameLayer3::resolveEnemy2TileCollision(Enemy2* enemy, Tile* tile) {
	float collisionScale = 0.20f;
	float adjustedWidth = enemy->width * collisionScale;
	float adjustedHeight = enemy->height * collisionScale;
...
void GameLayer3::handlePlayerCollisions() {
...
void GameLayer3::handleCollectables() {
...
void GameLayer3::handleProjectileCollisions() {
...
void GameLayer3::calculateScroll() {
...
void GameLayer3::keysToControls(SDL_Event event) {
	if (event.type == SDL_KEYDOWN) {
